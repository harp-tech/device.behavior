...
- 3 poke ports, each combining an infrared beam input, an LED driver, a 12 V valve driver, and a general-purpose digital line.
...
- Poke ports: 3 (IR input, LED, 12 V valve, digital IO per port)
...
- LED outputs: 2 current-controlled (2 – 100 mA) + 2 RGB
- Camera triggers: 2 (on **DO0**/**DO1**, 2 – 600 Hz)
- Servo outputs: 2 (on **DO2**/**DO3**, period and pulse width in µs)
- Quadrature encoder inputs: 1 (on Port 2, read at 1 kHz)
